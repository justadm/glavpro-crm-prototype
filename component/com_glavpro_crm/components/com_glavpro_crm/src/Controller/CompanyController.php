...
final class CompanyController extends BaseController
{
    public function createDemo(): void
    {
        ...
        if ($user->guest) {
            $app->enqueueMessage('Для работы с CRM нужно войти.', 'warning');
        ...
        $count = max(1, (int) $app->input->getInt('count', 1));
        $count = min($count, 50);
    ...
    public function addEvent(): void
    {
        ...
        $companyId = (int) $app->input->getInt('company_id');
        $eventType = (string) $app->input->getString('event_type');
        ...
        // Ensure event is allowed for current stage (protect from tampering).
        ...
        $availableActions = $engine->getAvailableActions((string) $company->stage_code, $events);

        $actionToEvent = [
            'call' => EventTypes::ATTEMPT_CONTACT,
            'comment_after_call' => EventTypes::LPR_CALL_DONE,
            'fill_discovery' => EventTypes::DISCOVERY_FILLED,
            'schedule_demo' => EventTypes::DEMO_SCHEDULED,
            'do_demo_via_link' => EventTypes::DEMO_DONE,
            'create_application' => EventTypes::APPLICATION_CREATED,
            'send_commercial_offer' => EventTypes::COMMERCIAL_OFFER_SENT,
            'issue_invoice' => EventTypes::INVOICE_ISSUED,
            'mark_payment_received' => EventTypes::PAYMENT_RECEIVED,
            'issue_first_certificate' => EventTypes::FIRST_CERTIFICATE_ISSUED,
        ];

        $allowedEventTypes = [];
        foreach ($availableActions as $actionKey) {
            if (isset($actionToEvent[$actionKey])) {
                $allowedEventTypes[] = $actionToEvent[$actionKey];
            }
        }

        if (!\in_array($eventType, $allowedEventTypes, true)) {
            $app->enqueueMessage('Это действие недоступно на текущей стадии', 'warning');
            $this->setRedirect(Route::_('index.php?option=com_glavpro_crm&view=company&id=' . $companyId, false));
            return;
        }

        $payload = $this->buildPayload($eventType, $app->input);
        ...
        // Re-load after inserting the new event.
        $events = $this->loadEvents($companyId);
        ...
        if ($nextStage !== null) {
            ...
            // Still touch updated_at so the company bubbles up in lists.
            $touch = $db->getQuery(true)
                ->update($db->quoteName('#__glavpro_companies'))
                ->set($db->quoteName('updated_at') . ' = :updated')
                ->where($db->quoteName('id') . ' = :id')
                ->bind(':updated', $now)
